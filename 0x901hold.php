...
$sql = "SELECT trackingid FROM tracking WHERE trackingid = '" . $_SESSION["trackingid"] . "'";
...
if (isset($_POST['delete'])) {
	...
	$sql = "DELETE FROM tracking WHERE trackingid='" . $_SESSION["trackingid"] . "'";
		if ($conn->query($sql) === TRUE) {
			$smsg = "Trackingid deleted successfully...";
		}else{
			$fmsg = "Error deleting trackingid..." . $conn->error;
		}$conn->close();
}
...
if (isset($_POST['hold'])) {
	...
	$sql = "UPDATE tracking SET c='$c' WHERE trackingid = '" . $_SESSION["trackingid"] . "'";
	if ($conn->query($sql) === TRUE) {
		$xmsg = "Action was successful...";
	}else{
		$fxmxg = "An error occured...." . $conn->error;
...
?>
                ...
                <h4 class="modal-title" style="color: red;">STOP/HOLD A PACKAGE BELOW(for Manual or semi auto only)</h4>
                <?php if (isset($smsg)) { ?><div style="color: green;"><b><?php echo $smsg; ?></b></div>
                	<?php
                }?>
                <?php if (isset($fmsg)) { ?><div style="color: red;"><b><?php echo $fmsg; ?></b></div>
                	<?php
                }?>
                <?php if (isset($xmsg)) { ?><div style="color: green;"><b><?php echo $xmsg; ?></b></div>
                	<?php
                }?>
                <?php if (isset($fxmxg)) { ?><div style="color: red;"><b><?php echo $fxmxg; ?></b></div>
                	<?php
                }?>
                <h4 class="modal-title"> Welcome <?php echo $receiversname; ?></h4><br /><br /><br />
                ...
                <center>
                	...
                	<button type="submit" class="btn btn-danger" name="delete">DELETE PACKAGE</button><br /><br /><br />
                	<input type="" name="c" placeholder="input '1' to hold package"><br />
                	<button type="submit" class="btn btn-primary" name="hold">HOLD</button><br /><br />

                	<br><br><a href="index.php"><b>Click here to continue to our website</b></a>
                	</center>
               </form>
